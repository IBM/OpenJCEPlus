    ...
    dir("openjceplus/OpenJCEPlus") {
        if (software == "zos") {
            ...
            if ((OPENJCEPLUS_REPO == "") && (OPENJCEPLUS_BRANCH == "")) {
                ...
                deleteDir()
                if (env.CHANGE_FORK) {
                    sh "git clone -b ${GIT_BRANCH_NAME} git@github.com:${CHANGE_FORK}/OpenJCEPlus.git ."
                } else {
                    sh "git clone -b ${GIT_BRANCH_NAME} git@github.com:IBM/OpenJCEPlus.git ."
                }
            } else {
...
def getPlatforms() {
    ...
    if (S390X_ZOS == "true") {
        if (!env.CHANGE_TARGET.equals("java25")) {
