 ...
 * Signature: (JI)J
 */
JNIEXPORT jlong JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DHKEY_1generate__JI
 ...
 * Signature: (JI)[B
 */
JNIEXPORT jbyteArray JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DHKEY_1generateParameters
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jint numBits)
{
  static const char * functionName = "NativeInterface.DHKEY_generateParameters";
...
#ifdef DEBUG_DH_DETAIL
      if ( debug) {
        gslogMessage("DETAIL_DH FAILURE ICC_i2d_DHparams size=%d", (int) size);
      }
#endif
...
#ifdef DEBUG_DH_DATA
            if ( debug ) {
              gslogMessage("DETAIL_DH ICC_i2d_DHparams size=%d", (int) size);
 ...
 * Signature: (J[B)J
 */
JNIEXPORT jlong JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DHKEY_1generate__J_3B
...
#ifdef DEBUG_DH_DETAIL
        if ( debug ) {
          gslogMessage("DETAIL_DH returning dhKeyId=%lx", (long) ockDH);
        }
#endif
      }
    }
 ...
 * Signature: (J[B)J
 */
JNIEXPORT jlong JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DHKEY_1createPrivateKey
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jbyteArray privateKeyBytes)
{
  ...
  if (privateKeyBytes == NULL) {
	  throwOCKException(env, 0, "The specified DH Private Key bytes are incorrect.");
    if ( debug ) {
	  gslogFunctionExit(functionName);
	}
	return dhKeyId;
  ...
  if( NULL == keyBytesNative ) {
    ...
    if ( debug ) {
      ...
      if( NULL == ockPKey ) {
        ...
        ICC_EVP_PKEY * ret = ICC_d2i_PrivateKey(ockCtx, ICC_EVP_PKEY_DH, &ockPKey, &pBytes, (long)size);
        if( ret == NULL ) {
          ockCheckStatus(ockCtx);
#ifdef DEBUG_DH_DETAIL
          if ( debug ) {
 ...
 * Signature: (J[B)J
 */
JNIEXPORT jlong JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DHKEY_1createPublicKey
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jbyteArray publicKeyBytes)
{
  ...
  if (publicKeyBytes == NULL) {
 	  throwOCKException(env, 0, "The specified DH Public Key bytes are incorrect.");
    if ( debug ) {
 	  gslogFunctionExit(functionName);
 	}
 	return dhKeyId;
  ...
  if( NULL == keyBytesNative ) {
      ...
      if( NULL == ockPKey ) {
        ...
        ICC_EVP_PKEY * ret = ICC_d2i_PUBKEY(ockCtx, &ockPKey, (const unsigned char **) &pBytes, (long)size);
        if( ret == NULL ) {
          ockCheckStatus(ockCtx);
#ifdef DEBUG_DH_DETAIL
          if ( debug ) {
 ...
 * Signature: (JJ)[B
 */
JNIEXPORT jbyteArray JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DHKEY_1getParameters
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jlong dhKeyId)
{
  ...
  if (ockDH == NULL) {
 	  throwOCKException(env, 0, "The specified DH Key identifier is incorrect.");
    if ( debug ) {
 	  gslogFunctionExit(functionName);
 	}
 	return retParmBytes;
...
#ifdef DEBUG_DH_DETAIL
          if ( debug ) {
            gslogMessage("DETAIL_DH FAILURE ICC_i2d_DHparams size=%d", size);
          }
#endif
          throwOCKException(env, 0, "ICC_i2d_DHParams failed");
        } else {
 ...
 * Signature: (JJ)[B
 */
JNIEXPORT jbyteArray JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DHKEY_1getPrivateKeyBytes
  (JNIEnv * env, jclass thisObj, jlong ockContextId, jlong dhKeyId)
{
  static const char * functionName = "NativeInterface.DHKEY_getPrivateKeyBytes";
...
#ifdef DEBUG_DH_DETAIL
        if ( debug ) {
          gslogMessage("DETAIL_DH FAILURE ICC_i2d_PrivateKey size=%d", size);
        }
#endif
        ...
        if( keyBytesNative == NULL ) {
          ...
          if( size <= 0 ) {
            ...
            if ( debug ) {
              ...
              gslogMessageHex ((char *) keyBytes, 0, size, 0, 0, NULL);
            }
#endif
          }
        }
 ...
 * Signature: (JJ)[B
 */
JNIEXPORT jbyteArray JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DHKEY_1getPublicKeyBytes
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jlong dhKeyId )
{
 ...
 * Signature: (JJ)J
 */
JNIEXPORT jlong JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DHKEY_1createPKey
  (JNIEnv * env, jclass thisObj, jlong ockContextId, jlong dhKeyId)
{
...
#ifdef DEBUG_DH_DETAIL
      if ( debug ) {
        gslogMessage("DETAIL_DH FAILURE ICC_EVP_PKEY_set1_DH rc=%d", rc);
      }
#endif
 ...
 * Signature: (JJJ)[B
 */
JNIEXPORT jbyteArray JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DHKEY_1computeDHSecret
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jlong pubKeyId, jlong privKeyId)
 {
  ...
  keylen = ICC_BN_num_bytes(ockCtx,
  ICC_DH_get_PublicKey(ockCtx, ockPubDHKey));
  if (keylen <= 0) {
    ockCheckStatus(ockCtx);
#ifdef DEBUG_DH_DETAIL
    if ( debug ) {
      gslogMessage("DETAIL_DH FAILURE ICC_BN_num_bytes keylen=%d", keylen);
    }
#endif
    ...
    keylen = ICC_BN_bn2bin(ockCtx,
        ICC_DH_get_PublicKey(ockCtx, ockPubDHKey), bbbuf);
    if (keylen <= 0) {
      ockCheckStatus(ockCtx);
#ifdef DEBUG_DH_DETAIL
      if ( debug ) {
        gslogMessage("DETAIL_DH FAILURE ICC_BN_bn2bin keylen=%d", keylen);
      }
#endif
...
#ifdef DEBUG_DH_DETAIL
          if ( debug ) {
            gslogMessage("DETAIL_DH FAILURE secretBytes allocation");
          }
#endif
          throwOCKException(env, 0, "NewByteArray failed");
        } else {
...
#ifdef DEBUG_DH_DETAIL
            if ( debug ) {
              gslogMessage("DETAIL_DH FAILURE secretBytesNative allocation");
            }
#endif
...
#ifdef DEBUG_DH_DETAIL
              if ( debug ) {
                gslogMessage("DETAIL_DH FAILURE ICC_DH_compute_key=%d", lena);
              }
#endif
              throwOCKException(env, 0,
...
#ifdef DEBUG_DH_DETAIL
              if ( debug ) {
                gslogMessage("DETAIL_DH lena from ICC_DH_compute_key lena==%d",lena);
              }
#endif
                /* OCK sometimes computes secrets that are smaller than the key size;  
                   pad the leading bytes with zero to get the length to keylen
                 */ 
              if (lena < keylen) 
              {
...
#ifdef DEBUG_DH_DETAIL
                  if ( debug ) {
                    gslogMessage("DETAIL_DH lena greater than keylen; Redo the secret");
                  }
#endif
                  /* Redo the secret. Last time we chopped the last byte 
                     we need to look at optimizing and allocate a little bit more
                     the first time if possible
                     */
                  
                  if (secretBytesNative != NULL) {
                    (*env)->ReleasePrimitiveArrayCritical(env,
                                                          secretBytes, secretBytesNative, 0);
                  }
                  if (secretBytes != NULL) {
...
#ifdef DEBUG_DH_DETAIL
                    if ( debug ) {
                      gslogMessage("DETAIL_DH FAILURE secretBytes");
                    }
#endif
                    throwOCKException(env, 0, "NewByteArray failed during recomputation");
                } else {
                  secretBytesNative =
                    (unsigned char*) ((*env)->GetPrimitiveArrayCritical(
                      env, secretBytes, &isCopy));
                  if (secretBytesNative == NULL) {
                    ...
                    throwOCKException(env, 0,
                      ...
                      if (lena == -(ICC_ERROR_FLAG)) {
...
#ifdef DEBUG_DH_DETAIL
                        if ( debug ) {
                          gslogMessage("DETAIL_DH FAILURE ICC_DH_compute_key lena=%d", lena);
                        }
#endif
                        throwOCKException(env, 0,
                          "ICC_DH_compute_key failed");
                        ...
                        if ( debug ) {
                          gslogMessagePrefix("DETAIL_DH key Bytes :");
                          gslogMessageHex ((char *) secretBytes, 0, lena, 0, 0, NULL);
                        }
#endif
                        if (secretBytesNative != NULL) {
                          (*env)->ReleasePrimitiveArrayCritical(env,
 ...
 * Signature: (JJ)V
 */
JNIEXPORT void JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DHKEY_1delete
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jlong dhKeyId)
{
