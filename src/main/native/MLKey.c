  ...
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jstring cipherName)
{
  ...
  if (rv != ICC_OSSL_SUCCESS) {
   ...
   if (publen <= 0) {
      #ifdef DEBUG_PQC_KEY_DETAIL
      if (debug) {
        gslogMessage("ICC_i2d_PublicKey failure. Unable to get public key length\n");
      }
      #endif
      throwOCKException(env, 0, "ICC_i2d_PublicKey failure. Unable to get public key length for encoding");
   }
   ...
   if (rv <= 0) {
      #ifdef DEBUG_PQC_KEY_DETAIL
      if (debug) {
        gslogMessage("ICC_i2d_PublicKey failure. Unable to encode public key\n");
      }
      #endif
      free(pubdata);
      throwOCKException(env, 0, "ICC_i2d_PublicKey failure. Unable to get encoded public key");
      return mlkeyId;
   }
   ...
   if (rv <= 0) {
      ...
      #ifdef DEBUG_PQC_KEY_DETAIL
      if (debug) {
        gslogMessage("ICC_i2d_PrivateKey failure. Unable to encode private key\n");
      }
      #endif
      throwOCKException(env, 0, "CC_i2d_PrivateKey failure. Unable to get encoded private key");
      return mlkeyId;
   }
   ...
   if (!npa) {
      ...
      free(pubdata);
      free(privData);
      throwOCKException(env, 0, "ICC_d2i_PublicKey failure. Unable to reconstruct public key.");
      return mlkeyId;
   }
  
   if (1 != ICC_EVP_PKEY_cmp(ockCtx, pa, npa)) { /*compare pubkey and decoded key */
     ICC_EVP_PKEY_free(ockCtx, npa);
     free(pubdata);
     free(privData);      
     #ifdef DEBUG_PQC_KEY_DETAIL
     if (debug) {
       gslogMessage("warning - public key encode/decode missmatch\n For Alg = %s\n",algoChars);
     }
     #endif
   ...
   if (!npa) {
      ...
      #endif  
      throwOCKException(env, 0, "ICC_d2i_PrivateKey failure. Unable to reconstruct private key.");
      return mlkeyId;
   }
  ...
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jstring cipherName, jbyteArray privateKeyBytes)
{
  ...
  if( NULL == keyBytesNative ) {
    ...
    if (!nid) {
      throwOCKException(env, 0, "Algorithm not found."); /* Unsupported algorithm */
    } else {
      FILE* fp_rsp = stdout;

      ockPKey = ICC_d2i_PrivateKey(ockCtx, nid, &ockPKey, &pBytes, (long)size);
