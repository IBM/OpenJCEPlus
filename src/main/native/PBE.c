...
Java_com_ibm_crypto_plus_provider_ock_NativeInterface_PBE_1doFinal(
    JNIEnv* env, jclass thisObj, jlong contextId, jbyteArray algorithm,
    ...
    // Get the OID.
    text = (*env)->GetPrimitiveArrayCritical(env, algorithm, &isCopy);
    if (NULL == text) {
    ...
    if (0 == id) {
        ...
        if (debug) {
            gslogMessage(
                "DEBUG_PBE_DETAIL: Failed to get the NID of PBE algorithm");
        }
#endif
     ...
     * allowed, a padded ouput is expected for null input data for algorithms
     * with PKCS5Padding.
     */
...
cleanup:
    if (NULL != text) {
        (*env)->ReleasePrimitiveArrayCritical(env, algorithm, (void*)text, 0);
        text = NULL;
    }
    if (NULL != saltNative) {
