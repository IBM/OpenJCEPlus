 ...
 */
JNIEXPORT jlong JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_HKDF_1create
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jstring digestAlgo)
{
  ...
  if (digestAlgo == NULL) {
    throwOCKException(env, 0, "The specified digest algorithm is incorrect.");
	if( debug ) {
 ...
 * Signature: (JJ[BII)V
 */
JNIEXPORT  jbyteArray JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_HKDF_1extract
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jlong hkdfId, jbyteArray salt, jlong saltLenl, jbyteArray inKey, jlong inKeyLenl )
{
  ...
  if (ockHKDF == NULL) {
      throwOCKException(env, 0, "The specified HKDF identifier is incorrect.");
  	  if( debug ) {
  	    gslogFunctionExit(functionName);
  	  }
  	  return retPrk;
...
#ifdef DEBUG_HKDF_DETAIL
  if( debug ) {
    gslogMessage("DETAIL_HKDFextract  hkdfId : %lx saltLen %ld inKeyLen %ld", (long) hkdfId, (long) saltLen, (long) inKeyLen);
  }
#endif

  saltNative = (unsigned char*)((*env)->GetPrimitiveArrayCritical(env, salt,  &isCopy));
  if( NULL == saltNative) {
#ifdef DEBUG_HKDF_DETAIL
    if( debug ) {
    ...
    inKeyNative = (unsigned char*)((*env)->GetPrimitiveArrayCritical(env, inKey,  &isCopy));
    if( NULL == inKeyNative) {
#ifdef DEBUG_HKDF_DETAIL
      if( debug ) {
      ...
      if( debug ) {
        gslogMessagePrefix("DATA_HKDF %d inKey bytes length  : ", (int)inKeyLen);
        gslogMessageHex((char *) inKeyNative, 0, (int)inKeyLen, 0, 0, NULL);
        gslogMessagePrefix("DATA_HKDF %d salt bytes length  : ", (int)saltLen);
...
#endif
      ICC_HKDF_Extract(ockCtx,ockHKDF->md,saltNative,(int) saltLen,inKeyNative, (int) inKeyLen, prkLocal, &prkLen);
      ...
      if( prk == NULL ) {
        ...
        prkNative = (unsigned char*)((*env)->GetPrimitiveArrayCritical(env, prk, &isCopy));
        if( prkNative == NULL ) {
#ifdef DEBUG_HKDF_DETAIL
          if( debug ) {
...
#ifdef DEBUG_HKDF_DETAIL
          if( debug ) {
            gslogMessage("DETAIL_HKDF memcpy prkNative %lx prkLocal %lx prkLen %d", prkNative, prkLocal, prkLen);
          }
#endif
 ...
 * Signature: (JJ[BII)V
 */
JNIEXPORT  jbyteArray JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_HKDF_1expand
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jlong hkdfId, jbyteArray prk, jlong prkLenl, jbyteArray info, jlong infoLenl, jlong okmLenl )
{
  ...
  if ((ockHKDF == NULL) || (prk == NULL) || (info == NULL)) {
    throwOCKException(env, 0, "The specified HKDF identifiers are incorrect.");
	if( debug ) {
	  gslogFunctionExit(functionName);
	}
	return retOkm;
...
#ifdef DEBUG_HKDF_DETAIL
  if( debug ) {
    gslogMessage("DETAIL_HKDF hkdfId : %lx prkLen %ld infoLen %ld okmLen %ld", (long) hkdfId, (long) prkLen, (long) infoLen, (long) okmLen);
  }
#endif
  ...
  if( NULL == prkNative) {
    ...
    infoNative = (unsigned char*)((*env)->GetPrimitiveArrayCritical(env, info,  &isCopy));
    if( NULL == infoNative) {
#ifdef DEBUG_HKDF_DETAIL
      if( debug ) {
...
#ifdef DEBUG_HKDF_DATA
      if( debug ) {
        gslogMessagePrefix("DATA_HKDF %ld info bytes length  : ", (long)infoLen);
        gslogMessageHex((char *) infoNative, 0, (long)infoLen, 0, 0, NULL);
        gslogMessagePrefix("DATA_HKDF %ld prk bytes length  : ", (long)prkLen);
      ...
      ptr =  (unsigned char *) ICC_HKDF_Expand (ockCtx,ockHKDF->md,prkNative,(int) prkLen,infoNative,  infoLen, okmLocal, okmLen);
      if (ptr == NULL ) {
...
#ifdef DEBUG_HKDF_DETAIL
          if( debug ) {
            gslogMessage("DETAIL_HKDF FAILURE to allocate okm (hkdf)");
          }
#endif
          throwOCKException(env, 0, "NewByteArray failed");
        } else {
          okmNative = (unsigned char*)((*env)->GetPrimitiveArrayCritical(env, okm, &isCopy));
          if( okmNative == NULL ) {
#ifdef DEBUG_HKDF_DETAIL
            if( debug ) {
              gslogMessage("DETAIL_HKDF FAILURE to allocate okmNative");
            }
#endif
            throwOCKException(env, 0, "NULL from GetPrimitiveArrayCritical");
          } else {
#ifdef DEBUG_HKDF_DETAIL
            if( debug ) {
              gslogMessage("DETAIL_HKDF memcpy okmNative %lx okmLocal %lx okmLen %d", okmNative, okmLocal, okmLen);
            }
#endif
 ...
 * Signature: (JJ[BII)V
 */
JNIEXPORT  jbyteArray JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_HKDF_1derive
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jlong hkdfId, jbyteArray salt, jlong saltLenl, jbyteArray inKey, jlong inKeyLenl, jbyteArray info, 
  jlong infoLenl, jlong resKeyLenl )
{
  ...
  if ((ockHKDF == NULL) || (inKey == NULL)) {
    throwOCKException(env, 0, "The specified HKDF identifier or HKDF Key bytes are incorrect.");
  	if( debug ) {
  	  gslogFunctionExit(functionName);
  	}
  	return retResKey;
...
#ifdef DEBUG_HKDF_DETAIL
  if( debug ) {
    gslogMessage("DETAIL_HKDFextract  hkdfId : %lx saltLen %ld inKeyLen %ld infoLen %ld resKeyLen %ld",
      (long) hkdfId, (long) saltLen, (long) inKeyLen, (long) infoLen, (long) resKeyLen);
  }
#endif
    ...
    if( NULL == inKeyNative) {
      ...
      if ( NULL == infoNative) {
        ...
        if ( debug ) {
          ...
          gslogMessagePrefix("DATA_HKDF %d salt bytes length  : ", (int)saltLen);
          gslogMessageHex((char *) saltNative, 0, (int)saltLen, 0, 0, NULL);
          gslogMessagePrefix("DATA_HKDF %d info bytes length  : ", (int) infoLen);
          gslogMessageHex((char *) infoNative, 0, (int)infoLen, 0, 0, NULL);
        }
#endif
        ...
        ptr = ICC_HKDF (ockCtx,ockHKDF->md, saltNative,(long) saltLen,inKeyNative, (long) inKeyLen, infoNative, (long) infoLen, resKeyLocal, resKeyLen);
        if ( ptr == NULL ) {
...
#ifdef DEBUG_HKDF_DATA
          if ( debug ) {
            gslogMessage("DATA_HKDF Derived resKeyLen : %ld ", (long) resKeyLen);
            gslogMessagePrefix("DATA_HKDF Extracted Bytes : ");
            gslogMessageHex((char *) resKeyLocal, 0, resKeyLen, 0, 0, NULL);
          }
#endif
...
#ifdef DEBUG_HKDF_DETAIL
            if( debug ) {
              gslogMessage("DETAIL_HKDF FAILURE to allocate resKey(hkdf)");
            }
#endif
            throwOCKException(env, 0, "NewByteArray failed");
          } else {
            resKeyNative = (unsigned char*)((*env)->GetPrimitiveArrayCritical(env, resKey, &isCopy));
...
#ifdef DEBUG_HKDF_DETAIL
              if ( debug ) {
                gslogMessage("DETAIL_HKDF memcpy resKeyNative %lx resKeyLocal %lx resKeyLen %d", resKeyNative, resKeyLocal, resKeyLen);
              }
#endif
 ...
 * Signature: (JJ)I
 */
JNIEXPORT jint JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_HKDF_1size
 ...
 * Signature: (JJ)V
 */
JNIEXPORT void JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_HKDF_1delete
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jlong hkdfId)
{
