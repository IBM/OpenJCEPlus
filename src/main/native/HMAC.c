 ...
 */
JNIEXPORT jlong JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_HMAC_1create
...
int HMAC_init_internal (ICC_CTX *ockCtx, OCKHMAC *ockHMAC, unsigned char *keyNative, int keySize) {
    ...
    if( debug ) {
        ...
        if (NULL != ockHMAC )  {
            gslogMessage("DETAIL_HMAC ockHMAC->hmacCtx : %lx ockHMAC->md %lx", ockHMAC->hmacCtx, ockHMAC->md);
        }
    }
#endif
    ...
    rc = ICC_HMAC_Init(ockCtx, ockHMAC->hmacCtx, keyNative, keySize, ockHMAC->md);
#ifdef DEBUG_HMAC_DETAIL
    if( debug ) {
        gslogMessage("DETAIL_HMAC keysize : %d rc %d from ICC_HMAC_Init", (int) keySize, rc);
    }
#endif
 ...
 * It is called by method HMAC_update and HMAC_doFinal when an object have not been initialized.
 */
int HMAC_init
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jlong hmacId, jbyteArray key, jint keySize)
{
...
#endif
  if (ockHMAC == NULL) {
    throwOCKException(env, 0, "The specified HMAC Key identifier is incorrect.");
    if( debug ) {
      gslogFunctionExit(functionName);
  	}
    return FAIL_HMAC_INTERNAL_INIT;
...
/* update internal */
JNIEXPORT int HMAC_update_internal(ICC_CTX *ockCtx, OCKHMAC *ockHMAC, unsigned char *keyNative, int keySize,
                                  unsigned char *inputNative, int inputLen, bool needInit) {
    ...
    if (ockHMAC->hmacCtx == NULL) {
    	...
    	rc = ICC_HMAC_Update(ockCtx, ockHMAC->hmacCtx, inputNative, (int)inputLen);
#ifdef DEBUG_HMAC_DETAIL
    	if( debug ) {
 ...
 * Signature: (JJ[BII)V
 */
JNIEXPORT jint JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_HMAC_1update
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jlong hmacId, jbyteArray key, jint keyLength, 
  jbyteArray input, jint inputOffset, jint inputLen, jboolean needInit)
{
  ...
  if ((ockHMAC == NULL) || (key == NULL) || (keyLength <= 0)  || (input == NULL)) {
...
#ifdef DEBUG_HMAC_DETAIL
  if( debug ) {
    gslogMessage("DETAIL_HMAC hmacId : %lx inputLen %d inputOffset %d", (long) hmacId, (int) inputLen, inputOffset);
  }
#endif
  ...
  if( NULL == inputNative || NULL == keyNative) {
    ...
    result = HMAC_update_internal(ockCtx, ockHMAC, keyNative, keyLength, inputNative+inputOffset, inputLen, needInit);
  }
  if( inputNative != NULL ) {
...
JNIEXPORT int HMAC_doFinal_internal(ICC_CTX *ockCtx, OCKHMAC *ockHMAC, unsigned char *keyNative, int keySize, 
...
#ifdef DEBUG_HMAC_DETAIL
    		if( debug ) {
    			gslogMessage("DETAIL_HMAC FAILURE ICC_HMAC_Final failed rc %d", rc);
    		}
#endif
    	}
    }
 ...
 * Signature: (JJ)[B
 */
JNIEXPORT jint JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_HMAC_1doFinal
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jlong hmacId, jbyteArray key, jint keyLength, jbyteArray hmac, jboolean needInit)
{
  ...
  if ((ockHMAC == NULL) || (key == NULL) || (keyLength <= 0) || (hmac == NULL)) {
    throwOCKException(env, 0, "The specified parameters are incorrect.");
    if( debug ) {
      gslogFunctionExit(functionName);
    }
    ...
    hmacNative = (unsigned char*)((*env)->GetPrimitiveArrayCritical(env, hmac, &isCopy));
    if( hmacNative == NULL ) {
#ifdef DEBUG_HMAC_DETAIL
          if( debug ) {
      ...
      throwOCKException(env, 0, "NULL from GetPrimitiveArrayCritical");
    	...
    	result = HMAC_doFinal_internal(ockCtx, ockHMAC, keyNative, keyLength, hmacNative, needInit);
    }
  if( keyNative != NULL ) {
    (*env)->ReleasePrimitiveArrayCritical(env, key, keyNative, 0);
  }
 ...
 * Signature: (JJ)I
 */
JNIEXPORT jint JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_HMAC_1size
 ...
 * Signature: (JJ)V
 */
JNIEXPORT void JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_HMAC_1delete
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jlong hmacId)
{
