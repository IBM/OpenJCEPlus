...
//
int
gslogFunctionEntry( const char * functionName )
{
...
//
int
gslogError( const char * formatString, ... )
{
  int         charsPrinted;
  va_list     formatArgs;
...
//
int
gslogMessage( const char * formatString, ... )
{
  int         charsPrinted;
  va_list     formatArgs;
  static char printBuffer[4096];
...
//
int
gslogMessagePrefix( const char * formatString, ... )
{
  int         charsPrinted;
  va_list     formatArgs;
  static char printBuffer[4096];
...
//
int
gslogMessageHex( char bytes[], int offset, int length, int spaceAfter, int newlineAfter, char * newlinePrefix )
{
	int index;
        int charsPrinted = 0;

	for( index = 1; index <= length; index++) {
		charsPrinted += fprintf(stderr, "%2.2X", (unsigned char)bytes[offset+index-1]);
		if( (newlineAfter > 0)  && ((index % newlineAfter) == 0) && (index < length) ) {
...
//
int
gslogFunctionExit( const char * functionName )
{
...
//
void
throwOCKException( JNIEnv *env, int code, const char* msg )
{
  ...
  if( debug ) {
    gslogMessage("Throwing exception %s : code=%d, msg=\"%s\"", exceptionClass, code, (msg ? msg : ""));
  }
  ...
  if(msg) {
    msgCopy = (char *) malloc(strlen(msg)+1);    /* for some reason strdup just throws segvs when used against msg, so brute force. */
    if(msgCopy) {
...
#pragma convert("ISO8859-1")
#endif
    if( !(mid = (*env)->GetMethodID(env, clazz, "<init>", "(Ljava/lang/String;)V")) ) {
#ifdef __MVS__
#pragma convert(pop)
#endif
...
#pragma convert(pop)
#endif
      gslogError("Can't find default constructor for %s(int)", exceptionClass);
      return;
    }
