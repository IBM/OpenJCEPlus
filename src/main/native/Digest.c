 ...
 */
JNIEXPORT jlong JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DIGEST_1create
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jstring digestAlgo)
{
  ...
  if (digestAlgo == NULL){
    throwOCKException(env, 0, "Digest create failed. The specified digest algorithm is incorrect.");
	if( debug ) {
...
#ifdef DEBUG_DIGEST_DETAIL 
    if ( debug ) {
      gslogMessage ("DETAIL_DIGEST FAILURE JG_DEBUG ICC_EVP_get_digestbyname failed");
    }
#endif
...
#ifdef DEBUG_DIGEST_DETAIL
        if( debug ) {
          gslogMessage("DETAIL_DIGEST digestId=%lx ockDigest->mdCtx=%lx ockDigest->md=%lx", (long) digestId, ockDigest->mdCtx, ockDigest->md);
        }
#endif
      }
    }
 ...
 * Signature: (JJ[BII)V
 */
JNIEXPORT int DIGEST_update_internal(ICC_CTX *ockCtx, OCKDigest *ockDigest, unsigned char *dataNative, int dataLen) {
  int rc = ICC_OSSL_SUCCESS;
  static const char * functionName = "NativeInterface.DIGEST_update";
  if ((ockDigest == NULL) || (dataNative == NULL) || (dataLen < 0) || (ockDigest->mdCtx == NULL)) {
    if ( debug ) {
	  gslogFunctionExit(functionName);
	}
...
JNIEXPORT jint JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DIGEST_1update
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jlong digestId, jbyteArray data, jint offset, jint dataLen)
{
  static const char * functionName = "NativeInterface.DIGEST_update";
  ...
  if ((ockDigest == NULL) || (data == NULL) || (offset < 0)) {
    throwOCKException(env, 0, "Digest Update failed. The specified input parameters are incorrect.");
	if( debug ) {
	  gslogFunctionExit(functionName);
	}
...
#ifdef DEBUG_DIGEST_DETAIL
    if( debug ) {
      gslogMessagePrefix("DETAIL_DIGEST %d bytes to update offset %d: ", (int)dataLen, (int) offset);
...
#ifdef DEBUG_DIGEST_DETAIL
     if( debug ) {
       gslogMessage("DETAIL_DIGEST digestId=%lx ockDigest->mdCtx=%lx ", (long) digestId, ockDigest->mdCtx );
        }
#endif
    returnResult = DIGEST_update_internal(ockCtx, ockDigest, dataNative+offset, (int)dataLen);
    if( DIGEST_INTERNAL_SUCCESS > returnResult ) {
#ifdef DEBUG_DIGEST_DETAIL 
      if ( debug ) {
        gslogMessage ("DETAIL_DIGEST FAILURE ICC_EVP_DigestUpdate failed");
      }
#endif
      ockCheckStatus(ockCtx);
    }
 ...
 * Signature: (JJJI)V
 */
JNIEXPORT void JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DIGEST_1updateFastJNI
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jlong digestId, jlong dataBuffer, jint dataLen)
{
  ...
  rc = ICC_EVP_DigestUpdate(ockCtx, ockDigest->mdCtx, dataNative, (int)dataLen);
  if( ICC_OSSL_SUCCESS != rc ) {
    ockCheckStatus(ockCtx);
 ...
 * Signature: (JJ)[B
 */
JNIEXPORT jbyteArray JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DIGEST_1digest
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jlong digestId)
{
  ...
  if (ockDigest == NULL) {
	  throwOCKException(env, 0, "Digest digest failed. The specified Digest identifier is incorrect.");
  }
...
#ifdef DEBUG_DIGEST_DETAIL 
      if ( debug ) {
        gslogMessage ("DETAIL_DIGEST FAILURE Failed to allocate digestBytes");
      }
#endif
...
#ifdef DEBUG_DIGEST_DETAIL 
      if ( debug ) {
        gslogMessage ("DETAIL_DIGEST FAILURE Failed to allocate digestBytesNative");
      }
#endif
        throwOCKException(env, 0, "NULL from GetPrimitiveArrayCritical");
      } else {
        rc = ICC_EVP_DigestFinal(ockCtx, ockDigest->mdCtx, digestBytesNative, (unsigned int *) &digestLen);
        if( ICC_OSSL_SUCCESS != rc ) {
#ifdef DEBUG_DIGEST_DETAIL 
          if ( debug ) {
            gslogMessage ("DETAIL_DIGEST FAILURE ICC_EVP_DigestFinal failed");
          }
#endif
          ockCheckStatus(ockCtx);
...
#ifdef DEBUG_DIGEST_DETAIL
          if ( debug ) {
            gslogMessage("DETAIL_DIGEST ockDigest->mdCtx %lx : ", ockDigest->mdCtx);
            gslogMessagePrefix("DETAIL_DIGEST DigestLen %d : ", (int)digestLen);
            gslogMessageHex((char *) digestBytes, 0, (int)digestLen, 0, 0, NULL);
          }
#endif 
        }
      }
 ...
 * Signature: (JJJI)V
 */
JNIEXPORT void JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DIGEST_1digest_1and_1reset__JJJI
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jlong digestId,  jlong digestBytes, jint length)
{
  ...
  if ((ockDigest == NULL) || (digestBytesNative == NULL) || (length < 0) || (ockDigest->mdCtx == NULL)) {
    throwOCKException(env, 0, "Digest reset failed. The specified Digest identifier or the digest bytes are incorrect.");
	if ( debug ) {
	  gslogFunctionExit(functionName);
	}
	return;
  }
  rc = ICC_EVP_DigestFinal(ockCtx, ockDigest->mdCtx, digestBytesNative, &digestLen);
  if( ICC_OSSL_SUCCESS != rc ) {
    ockCheckStatus(ockCtx);
    throwOCKException(env, 0, "ICC_EVP_DigestFinal failed");
  }
 ...
 * Signature: (JJ[B)V
 */
JNIEXPORT int DIGEST_digest_and_reset_internal(ICC_CTX *ockCtx, OCKDigest *ockDigest, unsigned char *digestBytesNative) {
  int rc = ICC_OSSL_SUCCESS;
  static const char * functionName = "NativeInterface.DIGEST_digest_and_reset_internal";
...
JNIEXPORT jint JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DIGEST_1digest_1and_1reset__JJ_3B
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jlong digestId, jbyteArray digestBytes)
{
  static const char * functionName = "NativeInterface.DIGEST_digest_and_reset";
  ...
  if ((ockDigest == NULL) || (digestBytes == NULL)) {
    throwOCKException(env, 0, "Digest reset failed. The specified Digest identfier or the digest bytes are incorrect.");
 	if( debug ) {
 	  gslogFunctionExit(functionName);
 	}
 	return returnResult;
  ...
  if( digestBytesNative == NULL ) {
    ...
    returnResult = DIGEST_digest_and_reset_internal(ockCtx, ockDigest, digestBytesNative);
    if( DIGEST_INTERNAL_SUCCESS > returnResult ) {
      ockCheckStatus(ockCtx);
    }
  }
 ...
 * Signature: (JJ)V
 */
JNIEXPORT jint JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DIGEST_1size
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jlong digestId)
{
  ...
  if (ockDigest == NULL) {
    ...
    throwOCKException(env, 0, "Digest size calculation failed. The specified Digest identifier is incorrect.");
  }
  else if (ockDigest->md == NULL) {
	if( debug ) {
	  gslogFunctionExit(functionName);
	}
	throwOCKException(env, 0, "Digest size calculation failed. The specified Digest is incorrect.");
  }
  else {
    digestLen = ICC_EVP_MD_size(ockCtx, ockDigest->md);
  }
...
#ifdef DEBUG_DIGEST_DETAIL
  if ( debug ) {
     gslogMessage("DETAIL_DIGEST ockDigest->md=%lx digestLen %d: ", ockDigest->md, (int) digestLen);
  }
#endif 
  if( debug ) {
    gslogFunctionExit(functionName);
 ...
 * Signature: (JJ)V
 */
JNIEXPORT void JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DIGEST_1reset
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jlong digestId)
{
  ...
  if (ockDigest == NULL) {
    throwOCKException(env, 0, "Digest init operation failed. The specified Digest identifier is incorrect.");
	if( debug ) {
	  gslogFunctionExit(functionName);
	}
	return;
...
#ifdef DEBUG_DIGEST_DETAIL
  if ( debug ) {
     gslogMessage("DETAIL_DIGEST ockDigest->mdCtx=%lx digestId %lx ockDigest->md=%lx : ", ockDigest->mdCtx, (long) digestId, ockDigest->md);
  }
#endif 
 ...
 * Signature: (JJ)V
 */
JNIEXPORT void JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DIGEST_1delete
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jlong digestId)
{
...
#ifdef DEBUG_DIGEST_DETAIL 
		  if ( debug ) {
			  gslogMessage ("DETAIL_DIGEST FAILURE ICC_EVP_MD_CTX_free failed rc %d", rc);
		  }
#endif
		  ockCheckStatus(ockCtx);
