 ...
 */
JNIEXPORT jlong JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_POLY1305CIPHER_1create
 ...
 * Signature: (JJZ)V
 */
JNIEXPORT void JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_POLY1305CIPHER_1init
		...
		jbyteArray key, jbyteArray iv)
{
	...
	if ( (ockCipher == NULL) || (key == NULL)) {
	  throwOCKException(env, 0, "The specified Poly1305Cipher arguments are incorrect.");
	  if( debug ) {
	    gslogFunctionExit(functionName);
	  }
	  return;
 ...
 * Signature: (JJ)V
 */
JNIEXPORT void JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_POLY1305CIPHER_1clean
 ...
 * Signature: (JJZ)V
 */
JNIEXPORT void JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_POLY1305CIPHER_1setPadding
(JNIEnv *env, jclass thisObj, jlong ockContextId, jlong ockCipherId, jint paddingId)
{
	static const char * functionName = "NativeInterface.POLY1305CIPHER_setPadding";
	...
	if (ockCipher == NULL) {
	  ...
	  throwOCKException(env, 0, "The specified Poly1305Cipher identifier is incorrect.");
	  return;
	}
 ...
 * Signature: (JJ)I
 */
JNIEXPORT jint JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_POLY1305CIPHER_1getBlockSize
(JNIEnv *env, jclass thisObj, jlong ockContextId, jlong ockCipherId)
{
	static const char * functionName = "NativeInterface.POLY1305CIPHER_getBlockSize";
 ...
 * Signature: (JJ)I
 */
JNIEXPORT jint JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_POLY1305CIPHER_1getKeyLength
(JNIEnv *env, jclass thisObj, jlong ockContextId, jlong ockCipherId)
{
	static const char * functionName = "NativeInterface.POLY1305CIPHER_getKeyLength";
 ...
 * Signature: (JJ)I
 */
JNIEXPORT jint JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_POLY1305CIPHER_1getIVLength
(JNIEnv *env, jclass thisObj, jlong ockContextId, jlong ockCipherId)
{
	static const char * functionName = "NativeInterface.POLY1305CIPHER_getIVLength";
 ...
 * Signature: (JJ)I
 */
JNIEXPORT jint JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_POLY1305CIPHER_1getOID
(JNIEnv *env, jclass thisObj, jlong ockContextId, jlong ockCipherId)
{
 ...
 * Signature: (JJI[B[B)I
 */
JNIEXPORT jint JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_POLY1305CIPHER_1encryptUpdate
...
{
	static const char * functionName = "NativeInterface.POLY1305CIPHER_encryptUpdate";
    ...
    if ((ockCipher == NULL) || (plaintext == NULL)) {
      throwOCKException(env, 0, "The specified Poly1305Cipher encrypt update arguments are incorrect.");
      if( debug ) {
  		gslogFunctionExit(functionName);
  	  }
      return (jint) outLen;
	...
	if( NULL == plaintextNative ) {
		...
		if ( NULL != ciphertextNative ) {
			// Update cipher text...
			rc = ICC_EVP_EncryptUpdate(ockCtx, ockCipher->cipherCtx, ciphertextNative + ciphertextOffset,
					&outLen, plaintextNative + plaintextOffset, (int)plaintextLen);
		} else {
			// Update AAD...
			rc = ICC_EVP_EncryptUpdate(ockCtx, ockCipher->cipherCtx, NULL, &outLen,
		...
		if( rc != ICC_OSSL_SUCCESS ) {
			...
			if( debug ) {
				if (NULL != ciphertextNative) {
					...
					gslogMessageHex((char *) ciphertextNative, ciphertextOffset, outLen, 0, 0, NULL);
				} else {
					gslogMessagePrefix("AAD : ");
					gslogMessageHex((char *) plaintextNative, plaintextOffset, outLen, 0, 0, NULL);
				}
			}
#endif
 ...
 * Signature: (JJI[B[B)I
 */
JNIEXPORT jint JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_POLY1305CIPHER_1encryptFinal
(JNIEnv *env, jclass thisObj, jlong ockContextId, jlong ockCipherId, jbyteArray plaintext,
		jint plaintextOffset, jint plaintextLen, jbyteArray ciphertext, jint ciphertextOffset, jbyteArray tag)
{
	static const char * functionName = "NativeInterface.POLY1305CIPHER_encryptFinal";
	...
	if ((ockCipher == NULL) || ( ciphertext == NULL)) {
	  throwOCKException(env, 0, "The specified Poly1305Cipher encrypt final arguments are incorrect.");
	  if( debug ) {
	    gslogFunctionExit(functionName);
	  }
	...
	if( (NULL == ciphertextNative) || ((plaintextLen > 0) && (plaintextNative == NULL)) || (NULL == tagNative) ) {
		throwOCKException(env, 0, "NULL from GetPrimitiveArrayCritical!");
	} else {
		if( plaintextLen > 0 ) {
#ifdef DEBUG_CIPHER_DATA  	
			...
			rc = ICC_EVP_EncryptUpdate(ockCtx, ockCipher->cipherCtx, ciphertextNative + ciphertextOffset,
					&updateOutlen, plaintextNative + plaintextOffset, (int)plaintextLen);
			if( rc != ICC_OSSL_SUCCESS ) {
				ockCheckStatus(ockCtx);
				throwOCKException(env, 0, "ICC_EVP_EncryptUpdate failed!\n");
			} else {
				...
				if( debug ) {
					...
					gslogMessageHex((char *) ciphertextNative, ciphertextOffset, updateOutlen, 0, 0, NULL);
				}
#endif     	
			}
		}
		...
		if( rc == ICC_OSSL_SUCCESS ) {
			rc = ICC_EVP_EncryptFinal(ockCtx, ockCipher->cipherCtx, ciphertextNative + ciphertextOffset + updateOutlen,
					&finalOutlen);
			if( rc != ICC_OSSL_SUCCESS ) {
				ockCheckStatus(ockCtx);
				...
				if( debug ) {
					...
					gslogMessageHex((char *) ciphertextNative, ciphertextOffset+updateOutlen, finalOutlen, 0, 0, NULL);
	...
	rc = ICC_EVP_CIPHER_CTX_ctrl(ockCtx, ockCipher->cipherCtx, EVP_CTRL_AEAD_GET_TAG, POLY1305_TAG_SIZE, tagNative);

	if( rc != ICC_OSSL_SUCCESS ) {
		ockCheckStatus(ockCtx);
		throwOCKException(env, 0, "ICC_EVP_CIPHER_CTX_ctrl(EVP_CTRL_AEAD_GET_TAG) failed!\n");
	} else {
#ifdef DEBUG_CIPHER_DATA
		if( debug ) {
			gslogMessagePrefix("Cipher Tag : ");
			gslogMessageHex((char *) tagNative, 0, POLY1305_TAG_SIZE, 0, 0, NULL);
		}
#endif
	}
 ...
 * Signature: (JJI[B[B)I
 */
JNIEXPORT jint JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_POLY1305CIPHER_1decryptUpdate
...
{
	static const char * functionName = "NativeInterface.POLY1305CIPHER_decryptUpdate";
	...
	if ((ockCipher == NULL) || (ciphertext == NULL)) {
	  throwOCKException(env, 0, "The specified Poly1305Cipher decrypt update arguments are incorrect.");
	  if( debug ) {
	    gslogFunctionExit(functionName);
	  }
	  return (jint) outLen;
	...
	if( NULL == ciphertextNative ) {
		...
		if( debug ) {
			gslogMessagePrefix("CipherText : ");
			gslogMessageHex((char *) ciphertextNative, ciphertextOffset, ciphertextLen, 0, 0, NULL);
		}
#endif

		if ( NULL != plaintextNative ) {
			// Update plain text...
			rc = ICC_EVP_DecryptUpdate(ockCtx, ockCipher->cipherCtx, plaintextNative + plaintextOffset, &outLen,
					ciphertextNative + ciphertextOffset, (int)ciphertextLen);
		} else {
			// Update AAD...
			rc = ICC_EVP_DecryptUpdate(ockCtx, ockCipher->cipherCtx, NULL, &outLen,
					ciphertextNative + ciphertextOffset, (int)ciphertextLen);
		}
 ...
 * Signature: (JJI[B[B)I
 */
JNIEXPORT jint JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_POLY1305CIPHER_1decryptFinal
(JNIEnv *env, jclass thisObj, jlong ockContextId, jlong ockCipherId, jbyteArray ciphertext,
		jint ciphertextOffset, jint ciphertextLen, jbyteArray plaintext, jint plaintextOffset, jbyteArray tag)
{
	static const char * functionName = "NativeInterface.POLY1305CIPHER_decryptFinal";
	...
	if ((ockCipher == NULL) || ( plaintext == NULL)) {
      throwOCKException(env, 0, "The specified Poly1305Cipher decrypt final arguments are incorrect.");
	  if( debug ) {
	    gslogFunctionExit(functionName);
	  }
	  return (jint) finalOutlen;
	...
	if( (NULL == plaintextNative) || ((ciphertextLen > 0) && (ciphertextNative == NULL)) || (NULL == tagNative) ) {
		throwOCKException(env, 0, "NULL from GetPrimitiveArrayCritical!");
	} else {
		if( ciphertextLen > 0 ) {
			...
			rc = ICC_EVP_DecryptUpdate(ockCtx, ockCipher->cipherCtx, plaintextNative + plaintextOffset,
					&updateOutlen, ciphertextNative + ciphertextOffset, (int)ciphertextLen);
			...
			if( rc != ICC_OSSL_SUCCESS ) {
				...
				if( debug ) {
					...
					gslogMessageHex((char *) plaintextNative, plaintextOffset, updateOutlen, 0, 0, NULL);
				}
#endif
		...
		rc = ICC_EVP_CIPHER_CTX_ctrl(ockCtx, ockCipher->cipherCtx, EVP_CTRL_AEAD_SET_TAG, POLY1305_TAG_SIZE, tagNative);

		if( rc != ICC_OSSL_SUCCESS ) {
			ockCheckStatus(ockCtx);
			throwOCKException(env, 0, "ICC_EVP_CIPHER_CTX_ctrl(EVP_CTRL_AEAD_SET_TAG) failed!\n");
		} else {
		...
		if( rc == ICC_OSSL_SUCCESS ) {
			...
			rc = ICC_EVP_DecryptFinal(ockCtx, ockCipher->cipherCtx, plaintextNative + plaintextOffset + updateOutlen, &finalOutlen);
			...
			if ( rc != ICC_OSSL_SUCCESS ) {
				...
				if( debug ) {
					...
					gslogMessageHex((char *) plaintextNative, plaintextOffset+updateOutlen, finalOutlen, 0, 0, NULL);
 ...
 * Signature: (JJ)V
 */
JNIEXPORT void JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_POLY1305CIPHER_1delete
(JNIEnv *env, jclass thisObj, jlong ockContextId, jlong ockCipherId)
{
