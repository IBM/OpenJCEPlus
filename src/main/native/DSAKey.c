 ...
 * Signature: (JI)J
 */
JNIEXPORT jlong JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DSAKEY_1generate__JI
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jint numBits)
{
  ...
  if (numBits  <= 0) {
    throwOCKException(env, 0, "DSA Key generate failed. The input parameter is incorrect.");
	if( debug ) {
...
#endif
  ockDSA = ICC_DSA_generate_parameters(ockCtx, (int)numBits, NULL, 0, NULL, NULL, NULL, NULL);
#ifdef DEBUG_DSA_DETAIL
  if( debug ) {
 ...
 * Signature: (JI)[B
 */
JNIEXPORT jbyteArray JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DSAKEY_1generateParameters
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jint numBits)
{
  static const char * functionName = "NativeInterface.DSAKEY_generateParameters";
...
#ifdef DEBUG_DSA_DETAIL
        if( debug ) {
          gslogMessage ("DETAIL_DSA FAILURE Could not allocate parmBytes");
        }
#endif
...
#ifdef DEBUG_DSA_DETAIL
        if( debug ) {
          gslogMessage ("DETAIL_DSA FAILURE Could not allocate GetPrimitiveArrayCritical");
        }
#endif
 ...
 * Signature: (J[B)J
 */
JNIEXPORT jlong JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DSAKEY_1generate__J_3B
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jbyteArray parmBytes)
{
  ...
  if (parmBytes == NULL) {
    throwOCKException(env, 0, "DSA Key generate failed. The specified input parameters are incorrect.");
    if( debug ) {
      gslogFunctionExit(functionName);
  	}
  	return dsaKeyId;
...
#ifdef DEBUG_DSA_DETAIL
        if( debug ) {
          gslogMessage ("DETAIL_DSA FAILURE ICC_DSA_generate_key failed");
        }
#endif
        ockCheckStatus(ockCtx);
        throwOCKException(env, 0, "ICC_DSA_generate_key failed");
 ...
 * Signature: (J[B)J
 */
JNIEXPORT jlong JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DSAKEY_1createPrivateKey
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jbyteArray privateKeyBytes)
{
  ...
  if (privateKeyBytes == NULL) {
	throwOCKException(env, 0, "The specified DSA Private Key bytes are incorrect.");
	if( debug ) {
	  gslogFunctionExit(functionName);
	}
	return dsaKeyId;
 ...
 * Signature: (J[B)J
 */
JNIEXPORT jlong JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DSAKEY_1createPublicKey
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jbyteArray publicKeyBytes)
{
  ...
  if (publicKeyBytes == NULL) {
	throwOCKException(env, 0, "The specified DSA Public Key bytes are incorrect.");
	if( debug ) {
	  gslogFunctionExit(functionName);
	}
	return dsaKeyId;
 ...
 * Signature: (JJ)[B
 */
JNIEXPORT jbyteArray JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DSAKEY_1getParameters
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jlong dsaKeyId)
{
  ...
  if (ockDSA == NULL) {
	throwOCKException(env, 0, "The specified DSA Key identifier is incorrect.");
	if( debug ) {
	  gslogFunctionExit(functionName);
	}
	return retParmBytes;
 ...
 * Signature: (JJ)[B
 */
JNIEXPORT jbyteArray JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DSAKEY_1getPrivateKeyBytes
  (JNIEnv * env, jclass thisObj, jlong ockContextId, jlong dsaKeyId)
{
  static const char * functionName = "NativeInterface.DSAKEY_getPrivateKeyBytes";
...
#ifdef DEBUG_DSA_DATA
        if ( debug ) { 
          gslogMessage ("DATA_DSA private key bytes size=%d", (int) size);
 ...
 * Signature: (JJ)[B
 */
JNIEXPORT jbyteArray JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DSAKEY_1getPublicKeyBytes
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jlong dsaKeyId )
{
  static const char * functionName = "NativeInterface.DSAKEY_getPublicKeyBytes";
...
#ifdef DEBUG_DSA_DETAIL
          if( debug ) {
            gslogMessage ("DETAIL_DSA FAILURE ICC_i2d_DSAPublicKey ");
          }
#endif
          throwOCKException(env, 0, "ICC_i2d_DSAPublicKey failed");
        } else {
 ...
 * Signature: (JJ)J
 */
JNIEXPORT jlong JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DSAKEY_1createPKey
  (JNIEnv * env, jclass thisObj, jlong ockContextId, jlong dsaKeyId)
{
...
#ifdef DEBUG_DSA_DETAIL
    if ( debug ) {
      gslogMessage ("DETAIL_DSA FAILURE ICC_EVP_PKEY_set1_DSA failed rc %d ", rc);
    }
#endif
      ockCheckStatus(ockCtx);
 ...
 * Signature: (JJ)V
 */
JNIEXPORT void JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_DSAKEY_1delete
  (JNIEnv *env, jclass thisObj, jlong ockContextId, jlong dsaKeyId)
{
