 ...
 * Signature: (J[BJ)[B
 */
JNIEXPORT jbyteArray JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_RSASSL_1SIGNATURE_1sign
  (JNIEnv * env, jclass thisObj, jlong ockContextId, jbyteArray digest, jlong rsaKeyId) {
  ...
  if ((digest == NULL) || (ockRSA == NULL)) {
    throwOCKException(env, 0, "RSA SSL Signature input arguments are incorrect.");
	if( debug ) {
...
#ifdef DEBUG_SIGNATURE_RSASSL_DETAIL
    if ( debug ) {
      gslogMessage("DETAIL_SIG_RSASSL sigLen - %d digestLen %d", sigLen, (int) digestLength);
    }
#endif
    if( sigLen <= 0 ) {
#ifdef DEBUG_SIGNATURE_RSASSL_DETAIL
      ...
      if( sigBytesLocal == NULL ) {
        ...
        rc = ICC_RSA_sign(ockCtx, NID, digestBytesNative, digestLength, sigBytesLocal, &outLen, ockRSA);
#ifdef DEBUG_SIGNATURE_RSASSL_DETAIL
        if ( debug ) {
          gslogMessage("DETAIL_SIG_RSASSL rc from ICC_RSA_Sign - %d outLen %d", rc, outLen);
        }
#endif
        if( ICC_OSSL_SUCCESS != rc ) {
#ifdef DEBUG_SIGNATURE_RSASSL_DETAIL
          if ( debug ) {
            gslogMessage ("DETAIL_SIG_RSASSL FAILURE ICC_RSA_sign %d ", rc);
          }
#endif
          ockCheckStatus(ockCtx);
...
#ifdef DEBUG_SIGNATURE_RSASSL_DETAIL
          if ( debug ) {
            gslogMessagePrefix("DETAIL_SIG_RSASSL Digest - %d bytes\n", digestLength);
            gslogMessageHex((char *) digestBytesNative, 0, digestLength, 0, 0, NULL);

            gslogMessagePrefix("DETAIL_SIG_RSASSL Signature - %d bytes\n", outLen);
 ...
 * Signature: (J[BJ[B)Z
 */
JNIEXPORT jboolean JNICALL Java_com_ibm_crypto_plus_provider_ock_NativeInterface_RSASSL_1SIGNATURE_1verify
  (JNIEnv * env, jclass thisObj, jlong ockContextId, jbyteArray digest, jlong rsaKeyId, jbyteArray sigBytes, jboolean convert) {
  ...
  if ((digest == NULL) || (ockRSA == NULL)) {
    throwOCKException(env, 0, "RSA SSL Signature verification input arguments are incorrect.");
  	if( debug ) {
  	  gslogFunctionExit(functionName);
  	}
  	return verified;
  ...
  if(digestBytesNative == NULL) {
...
#ifdef DEBUG_SIGNATURE_RSASSL_DETAIL
    if ( debug ) {
      gslogMessage("DETAIL_SIG_RSASSL digestLength=%d", (int)digestLength);
    }
#endif
    ...
    if(sigBytesNative == NULL) {
...
#ifdef DEBUG_SIGNATURE_RSASSL_DETAIL
      if ( debug ) {
        gslogMessagePrefix("DETAIL_SIG_RSASSL Signature to verify %d bytes:\n", (int)size);
      ...
      rc = ICC_RSA_verify(ockCtx, NID, digestBytesNative, digestLength, sigBytesNative, (unsigned int)size, ockRSA);
      if( ICC_OSSL_SUCCESS == rc ) {
        verified = 1;
      } else {
#ifdef DEBUG_SIGNATURE_RSASSL_DETAIL
  ...
  if( debug ) {
#ifdef DEBUG_SIGNATURE_RSASSL_DETAIL
    gslogMessage("DETAIL_SIG_RSASSL Signature verification Result %d ", (int)verified);
#endif
    gslogFunctionExit(functionName);
  }
