...
public class BaseTestAES extends BaseTestCipher {
    ...
    // 14 bytes: PASSED
    final byte[] plainText14 = "12345678123456".getBytes();

    // 16 bytes: PASSED
    final byte[] plainText16 = "1234567812345678".getBytes();

    // 18 bytes: PASSED
    final byte[] plainText18 = "123456781234567812".getBytes();

    // 63 bytes: PASSED
    final byte[] plainText63 = "123456781234567812345678123456781234567812345678123456781234567"
            .getBytes();

    // 128 bytes: PASSED
    final byte[] plainText128 = "12345678123456781234567812345678123456781234567812345678123456781234567812345678123456781234567812345678123456781234567812345678"
            .getBytes();

    // 512, 65536, 524288 bytes, payload increment of 32 bytes (up to 16384 bytes) : PASSED
    Random r = new Random(5);
    int iteration = 0;
    final byte[] plainText512 = new byte[512];
    final byte[] plainText65536 = new byte[65536];
    final byte[] plainText524288 = new byte[524288];
    final byte[] plainText1048576 = new byte[1048576];
    final byte[] plainText16KB = new byte[16384];
    final byte[] plainText = plainText128; // default value
